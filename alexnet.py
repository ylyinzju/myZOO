...
def alexnet(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        params['classifier.6.weight'] = torch.zeros(kwargs['num_classes'], 4096).normal_(0, 0.01)
...
if __name__ == '__main__':
    # alexnet2(pretrained=True, num_classes=120, init=False)
    print '*'*50
    for name in compute_layer_names:
        model = part_alexnet(name, num_classes=120)
        for m in model.modules():
            print m
        print '-'*35
    for name in fc_compute_layer_names:
        model = part_alexnet(name, num_classes=120)
        for m in model.modules():
            print m
        print '-'*35
